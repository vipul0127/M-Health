...
def view_hl7(request, file_id):
    ...
    creds = ServiceAccountCredentials.from_json_keyfile_name('/mHealth/credentials.json', scope)  # Replace with your credentials file path
...
def display_csv(request, file_id):
    ...
    # Set up Google Sheets API credentials
    scope = ['https://spreadsheets.google.com/feeds', 'https://www.googleapis.com/auth/drive']
    creds = ServiceAccountCredentials.from_json_keyfile_name('/mHealth/credentials.json', scope)
    client = gspread.authorize(creds)
...
from django.contrib.auth.decorators import login_required
from django.shortcuts import render
from .models import GoogleSheet  # Assuming GoogleSheet is your model

@login_required
def view_files(request):
    if request.user.is_superuser:
        # Superuser sees all files
        google_sheets = GoogleSheet.objects.all().select_related('uploaded_by')  # Prefetch user data if needed
    else:
        # Normal user sees only their files
        google_sheets = GoogleSheet.objects.filter(uploaded_by=request.user)
