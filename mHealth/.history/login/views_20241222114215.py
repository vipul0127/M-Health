...
def display_csv(request, file_id):
    ...
    local_csv_path = '/Users/krishbhoruka/Downloads/mHealth/ppg_output/s10_walk (1).csv'
    ...
    try:
        ...
        return HttpResponse(f"Error accessing local CSV file: {e}", status=500)
    ...
    for i, (day_time, day_ppg) in enumerate(zip(daily_time, daily_ppg), start=1):
        ...
        fig.add_trace(go.Scatter(
            ...
            hovertemplate='Time: %{x|%H:%M:%S}<br>PPG: %{y:.2f}<extra></extra>'
        ))
