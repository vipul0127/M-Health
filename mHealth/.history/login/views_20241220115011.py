...
def time_to_milliseconds(time_str):
    try:
        ...
        if len(time_parts) == 3:
            ...
            milliseconds = int(seconds_and_millis[1]) if len(seconds_and_millis) > 1 else 0
...
#         '/Users/krishbhoruka/Downloads/mHealth/credentials.json', scope
#     )
#     client = gspread.authorize(creds)
...
#     search_milliseconds = time_to_milliseconds(search_time) if search_time else None
#     search_results = []
...
#             time_milliseconds = time_to_milliseconds(row['Time'])
#             if time_milliseconds is not None:
#                 try:
#                     ppg_value = float(row['PPG'])

#                     # Detect new day based on time resets
#                     if time_milliseconds < previous_milliseconds:
#                         # Calculate heart rate statistics for the current day
#                         hr_stats = calculate_heart_rate(np.array(current_day_ppg)) or {
...
#                         current_day_ppg, current_day_time = [], []

#                     # Format time for plotting
#                     formatted_time = str(timedelta(milliseconds=time_milliseconds))
#                     current_day_ppg.append(ppg_value)
#                     current_day_time.append(formatted_time)
#                     previous_milliseconds = time_milliseconds

#                     # Search within a 300-second window
#                     if search_milliseconds and abs(time_milliseconds - search_milliseconds) <= 300000:
#                         search_results.append({'time': formatted_time, 'ppg': ppg_value})
...
#         x_times = [
#             datetime.combine(datetime.today().date(), datetime.strptime(t, "%H:%M:%S.%f").time())
#             for t in day_time
#         ]
...
#         # Update layout with dynamic time ticks
#         fig.update_layout(
#             title=f'PPG vs Time for Day {i}',
#             xaxis=dict(
#                 title='Time',
#                 type='date',
#                 showgrid=True,
...
#                 gridcolor='lightgrey',
#                 range=[
#                     min(day_ppg) - 0.1 * (max(day_ppg) - min(day_ppg)),
#                     max(day_ppg) + 0.1 * (max(day_ppg) - min(day_ppg))
#                 ],
#             ),
#             height=600,
        ...
        if 'PPG' in row and 'Time' in row:
            ...
            if time_milliseconds is not None:
                try:
                    ...
                    # Detect new day transitions
                    if previous_milliseconds and time_milliseconds < previous_milliseconds:
