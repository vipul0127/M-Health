...
def display_csv(request, file_id):
    ...
    # Set up Google Sheets API credentials
    scope = ['https://spreadsheets.google.com/feeds', 'https://www.googleapis.com/auth/drive']
    creds = ServiceAccountCredentials.from_json_keyfile_name('/Users/krishbhoruka/Downloads/mHealth/credentials.json', scope)
    client = gspread.authorize(creds)
            ...
            if time_seconds is not None:
                try:
                    ...
                    if time_seconds < previous_seconds:
                        # Process heart rate calculation
                        if current_day_ppg and current_day_time:
                            # Convert time to seconds and find peaks
                            time_intervals = [time_to_seconds(t) for t in current_day_time]
                            peaks, _ = find_peaks(current_day_ppg, distance=50)  # Adjust 'distance' based on sampling rate
                            if len(peaks) > 1:
                                rr_intervals = [
                                    (time_intervals[peaks[i + 1]] - time_intervals[peaks[i]])
                                    for i in range(len(peaks) - 1)
                                ]
                                avg_rr = sum(rr_intervals) / len(rr_intervals)
                                heart_rate = round(60 / avg_rr, 2)
                            else:
                                heart_rate = None

                            daily_heart_rates.append(heart_rate)

                        daily_ppg.append(current_day_ppg)
                        daily_time.append(current_day_time)
                        daily_stats.append({
                            'min': min(current_day_ppg),
                            'max': max(current_day_ppg),
                            'avg': round(sum(current_day_ppg) / len(current_day_ppg), 2),
                            'median': statistics.median(current_day_ppg),
                            'mode': statistics.mode(current_day_ppg),
                        })
                        current_day_ppg = []
                        current_day_time = []

                    formatted_time = str(timedelta(seconds=time_seconds))
    ...
    if current_day_ppg and current_day_time:
        time_intervals = [time_to_seconds(t) for t in current_day_time]
        peaks, _ = find_peaks(current_day_ppg, distance=50)
        if len(peaks) > 1:
            rr_intervals = [
                (time_intervals[peaks[i + 1]] - time_intervals[peaks[i]])
                for i in range(len(peaks) - 1)
    ...
    for i, (day_time, day_ppg) in enumerate(zip(daily_time, daily_ppg), start=1):
        heart_rate = daily_heart_rates[i - 1]
        stats = daily_stats[i - 1]
        stats['heart_rate'] = heart_rate

        fig = go.Figure()
        # Add your plotting logic as is
